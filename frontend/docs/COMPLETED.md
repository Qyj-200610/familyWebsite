...
### 美食点单 (FoodOrder.tsx)
- [x] 菜品卡片展示真实照片（来自 `/image/` 目录），`loading="lazy"` 懒加载
- [x] 图片加载失败自动回退为 emoji 占位
- [x] 6 道真实菜品：家烧六月黄🦀、烤鸡翅🍗、盐水麻虾🦐、凉拌牛肉🥩、生菜沙拉🥗、雪梨鲍鱼🥣
- [x] 菜品分类筛选（热菜/凉菜/汤品，其余分类为空状态提示）
- [x] 空分类显示友好提示（"该分类暂无菜品"）
- [x] 购物车 — 数量增减、移除单品、清空
- [x] 邮件提交通知（QQ SMTP）
- [x] 下单确认弹窗 + 备注 + 成功/错误状态
- [x] 移动端响应式 — 购物车滑出面板 + FAB 浮动按钮
...
- [x] **DISHES 替换** — 从 23 道 emoji 占位菜品替换为 6 道带真实照片的菜品
  - 图片存放位置：`frontend/public/image/`（Vite 静态资源服务）
  - 菜品分类：热菜 3 道（家烧六月黄、烤鸡翅、盐水麻虾）、凉菜 2 道（凉拌牛肉、生菜沙拉）、汤品 1 道（雪梨鲍鱼）
- [x] **Dish 接口新增 `image` 字段** — 存储图片路径 `/image/菜名.jpg`
- [x] **卡片图片区** — emoji 占位改为 `<img>` 标签，`object-fit: cover` + hover 缩放效果
- [x] **图片加载兜底** — `onError` 时隐藏 `<img>` 并显示 emoji fallback（绝对定位居中 + hover 缩放）
- [x] **空分类提示** — "主食/饮品/甜点"无菜品时显示友好空状态（"该分类暂无菜品"）

### 文档同步更新

- [x] **COMPLETED.md** — 更新美食点单功能描述（本文）
- [x] **interface.md** — 点单示例更新为新菜品名称
- [x] **TODOLIST.md** — 菜品数量从 23 更新为 6
